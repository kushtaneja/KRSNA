...
//  KRSNA
//
//  Created by Kush Taneja on 15/12/16.
...
class QuotesCollectionViewController: UICollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        ref.child("new_quotes").queryOrderedByKey().observe(.childAdded, with:  { (snapshot) in
            self.postKey = snapshot.key
            FIRDatabase.database().reference().child("new_quotes").child((self.postKey)!).queryOrderedByKey().observeSingleEvent(of: .value, with: { (snap) in
...
extension QuotesCollectionViewController: UICollectionViewDelegateFlowLayout {
    //1
